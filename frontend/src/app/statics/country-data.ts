import { euCountryList } from 'app/models/haulage.model';
...
export const mapCountryNameToEntity = countries.reduce(
  (memo, item) => {
    memo[item.name] = item;
    return memo;
  },
  {} as Record<string, ItemOf<typeof countries>>,
);

export const euContries = euCountryList.map((item) => mapCountryNameToEntity[item.name]);
